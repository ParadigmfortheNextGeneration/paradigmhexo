...
let processor = function(file) {
  ...
  let isThumbnail = _.some(options.profiles, function(profile, profileName) {
    var testIt = new RegExp('-' + profileName).test(path.basename(file.path))
    return testIt;
  })
  if (isThumbnail) {
    return vow.resolve()
  ...
  return vow.all(
    ...
    _.map(options.profiles, function(profile, profileName) {
      ...
      var resize_dir = path.join(
        hexo.source_dir,
        path.dirname(file.path)+"/resized"
      )
      ...
      let destPath = path.join(
        hexo.source_dir,
        path.dirname(file.path)+"/resized",
        filename.join('.') + '-' + profileName + '.' + extension
      )
...
hexo.extend.filter.register('before_generate', function() {
  ...
  return vow.all(_.map(posts, function(post) {
    ...
    return vow.all(_.map(options.profiles, function(profile, profileName) {
      var filename  = path.basename(post.thumbnail).split('.');
      var extension = filename.pop();
      post[profileName] = [
        path.dirname(post.thumbnail),
        '/resized/',
        filename.join('.') + '-' + profileName + '.' + extension
      ].join('')
      return post.save()
